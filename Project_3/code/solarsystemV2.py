...
class solarsystem:
    ...
    def eulercromer(self, diffeq=gravity):
        """ 
    ...
    def velocityverlet(self, diffeq=gravity):
        for i in xrange(self.N - 1):
            for n in xrange(self.numPlanets):
                a1 = diffeq(self, n, i)       # Acceleration at t=t
                self.pos[n][i + 1] = self.pos[n][i] + self.vel[n][i] * self.h + 0.5 * a1 * self.h**2
                a2 = diffeq(self, n, i + 1)   # Acceleration at t=t+h
